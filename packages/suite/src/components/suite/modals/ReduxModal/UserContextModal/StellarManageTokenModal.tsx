...
type StellarManageTokenModalProps =
    ...
    | {
          ...
          symbol: NetworkSymbol;
          contractAddress: string;
          tokenBalance: string; // Required for deactivate mode
          onCancel: () => void;
      };
    ...
    const handleSubmitTrustline = async ({ selectedFee, feePerUnit }: FormState) => {
        ...
        try {
            ...
            const result = await dispatch(
                mode === 'activate'
                    ? activateTokenThunk(thunkPayload)
                    : deactivateTokenThunk(thunkPayload),
            );
            ...
            if (
                mode === 'activate'
                    ? activateTokenThunk.fulfilled.match(result)
                    : deactivateTokenThunk.fulfilled.match(result)
            ) {
                ...
                dispatch(
                    notificationsActions.addToast({
                        type: mode === 'activate' ? 'add-token-success' : 'remove-token-success',
                    }),
                );
                onCancel(); // Close modal on success
            } else if (
                mode === 'activate'
                    ? activateTokenThunk.rejected.match(result)
                    : deactivateTokenThunk.rejected.match(result)
            ) {
                ...
                if (mode === 'activate') {
                    ...
                    dispatch(
                        notificationsActions.addToast({
                            type: 'error',
                            error: translationString('TR_REMOVE_TOKEN_TOAST_ERROR', {
                                error: errorMessage,
                            }),
                        }),
                    );
